...
@Table(name = "le_order")
@Entity
public class LeisureOrder extends BaseTimeEntity implements Serializable {
    ...
    private List<LeisureOrderOption> options;

    @OneToMany(cascade = CascadeType.ALL, mappedBy = "order")
    @JsonManagedReference
    private List<LeisureOrderOptionHistory> historyOptions;
    ...
    public void cancelOption(List<Long> optionIds, String cancelReason, String chargedName) {
        for (LeisureOrderOption option : options) {
    ...
    @Builder
    public LeisureOrder(Long bpOrderId, Long yanOrderId, Long dealId, String dealName, Long customerId, List<LeisureOrderOption> options, OrderStatus status, PaymentCancelStatus paymentCancelStatus, String channel, String category, LocalDate reservedDate, BigDecimal totalCostPrice, BigDecimal totalCoverPrice, BigDecimal totalSalePrice, BigDecimal canceledPrice, BigDecimal creditCardAmount, BigDecimal accountAmount, BigDecimal welfarePointAmount) {
        this.id = id;
