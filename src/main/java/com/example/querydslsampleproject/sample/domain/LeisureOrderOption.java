...
@Getter
@NoArgsConstructor
@Table(name = "le_order_option")
@Entity
public class LeisureOrderOption extends BaseTimeEntity implements Serializable {
    ...
    @Column(precision = 19, scale = 0)
    private BigDecimal salePrice; // 실 판매가격 = gajago판매가
    ...
    @JsonBackReference
    private LeisureOrder order;

    private LocalDate expiredDate;

    @OneToMany(mappedBy = "orderOption", cascade = CascadeType.ALL)
    @JsonManagedReference
    private List<LeisureChannelDiscount> channelDiscounts;

    @Builder
    public LeisureOrderOption(String channel, long optionId, int quantity, BigDecimal coverPrice, BigDecimal costPrice, BigDecimal salePrice, OrderOptionStatus status, LocalDateTime canceledDate, LocalDateTime consumedDate, String cancelReason, String cancelFailMessage, String chargedName, OrderFailCode orderFailCode, ForcedOrderFailCode forcedOrderFailCode, String optionName, YanOrderRefundType refundType, long yanOrderedOptionId, LeisureOrder order) {
